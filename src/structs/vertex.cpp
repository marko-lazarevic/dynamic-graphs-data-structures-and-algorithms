...
int Vertex::HashToSlot(int neighbor_id) const {
    return (neighbor_id & (beta - 1));
}

int Vertex::FindSlot(int neighbor_id) const {
...
int Vertex::FindInsertSlot(int neighbor_id, bool& already_present) const {
...
void Vertex::RemoveNeighbor(int neighbor_id, bool preserve_order) {
    int slot = FindSlot(neighbor_id);
    if (slot == -1 || degree == 0) {
        return;
    }
...
void Vertex::SetId(int new_id) {
    id = new_id;
}

void Vertex::RemapNeighborIdsAfterVertexRemoval(int removed_vertex_id) {
    int write_index = 0;
